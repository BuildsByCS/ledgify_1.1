...
export default function RegisterForm({ onSuccess, onToast }) {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            reset();
            // Brief shake-celebrate then switch to login
            gsap.fromTo(formRef.current, { x: 0 }, {
                keyframes: { x: [0, -6, 6, -4, 4, 0] },
                duration: 0.4, ease: 'power2.inOut',
                onComplete: () => setTimeout(onSuccess, 350),
            });
        } catch (err) {
            gsap.to(btnRef.current, { scale: 1, duration: 0.2 });
                ...
                <button ref={btnRef} type="submit" className="btn-primary px-4 py-2.5 sm:px-5 sm:py-[11px] text-[0.8rem] sm:text-[0.85rem]" disabled={isLoading}>
                    {isLoading
                        ? <span className="flex items-center justify-center gap-2">
                            <span className="spinner w-[14px] h-[14px] sm:w-[15px] sm:h-[15px]" /> Creating account…
                        </span>
                        : 'Create Account'}
                </button>
            </div>
